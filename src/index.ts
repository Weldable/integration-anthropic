  ...
  if (result.properties && typeof result.properties === 'object') {
    ...
    for (const [key, val] of Object.entries(props)) {
      if (val && typeof val === 'object' && !Array.isArray(val)) {
        newProps[key] = injectAdditionalProperties(val as Record<string, unknown>)
      } else {
        newProps[key] = val
      }
    }
      ...
      execute: async (args, ctx) => {
        ...
        if (!ALLOWED_MODELS.has(model)) {
          throw new IntegrationValidationError(
            `Unknown model "${model}". Allowed: ${[...ALLOWED_MODELS].sort().join(', ')}`,
            'model'
          )
        }
        ...
        if (schema) {
          ...
          const jsonOutput: Record<string, unknown> = typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)
